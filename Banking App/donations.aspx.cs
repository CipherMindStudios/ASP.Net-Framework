    ...
    public partial class donations : System.Web.UI.Page
    {
        ...
        protected void don_con_Click(object sender, EventArgs e)
        {
            ...
            if (bal >= number)
            {
                ...
                id++;
                rd.Close();
                cmd = "insert into Transactions values(" + id + "," + data + "," + supplierid + ",'" + supplier_accno + "','" + user_acno + "','" + user_name + "','" + ddlBank.SelectedValue + "'," + txtAccountNumber.Text + ",'Donations',CONVERT(DATE, GETDATE()))";
                command = new SqlCommand(cmd, conn); ; command.ExecuteNonQuery();
                ScriptManager.RegisterStartupScript(this, GetType(), "showalert", "alert('Donation Made');", true);
